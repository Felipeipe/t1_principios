        ...
        with open(filepathClientes, "r+") as file1:
            ...
            if articulo in inv:
                if inv[articulo] != 0:
                    ...
                    data1[cliente[1]][2].append([len(data1[cliente[1]][2]) + 1, funcionesCliente.accion("venta", articulo, datetime.today(), precio).asdict()])
                    file1.seek(0)
        ...
        with open(filepathInventario, "r+") as file1:
            ...
            with open(filepathCatalogo, "r+") as file2:
                ...
                while i < len(data2):
                    ...
                    i += 1
                if i == len(data2) - 1: # si llegamos al final del diccionario y la carta/sobre no existe, se crea una nueva entrada en el catálogo
    ...
    # si es comando, se llama a la función correspondiente; si no, se envía cómo mensaje al cliente:
        ...
        if instructions == ':status:':
            ...
            if connectionStatus:
                N = len(comm)
                if N != 3:
                    sockEjecutivo.sendall("Formato invalido. Recuerde que el formato es :buy <carta> <precio>:\n".encode())
                else:                    
                    card = comm[1]
